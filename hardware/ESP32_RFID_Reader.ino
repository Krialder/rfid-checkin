/*
 * Compact ESP32 RFID Check-in System
 * Optimized for reliability and database integration
 * 
 * Features:
...
void setup() {
  ...
  Serial.println("=== ESP32 RFID Check-in ===");
...
void sendToServer(String rfidTag) {
  ...
  // Create fresh HTTP client to avoid state issues
  HTTPClient httpClient;
  httpClient.begin(SERVER_URL);
...
void handleServerError(String response) {
  ...
  Serial.println("System will continue - try next scan");
}

void handleConnectionError() {
  Serial.println("Connection failed - checking WiFi...");
  
  // Force WiFi status check
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("WiFi disconnected - attempting reconnection");
    wifiConnected = false;
    connectWiFi();
  } else {
    Serial.println("WiFi OK - server may be temporarily unavailable");
  }
}
...
void serialEvent() {
  if (Serial.available()) {
    ...
    if (cmd == "status") {
      ...
      Serial.println("Commands: status, test, restart, help");
    }
  }
}
